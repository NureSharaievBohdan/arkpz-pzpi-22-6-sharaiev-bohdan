...
string getJwtAcces(string email, string password) {
    ...
    if (curl) {
        CURLcode res;
        curl_easy_setopt(curl, CURLOPT_URL, Config::LOGIN_API.c_str());
...
vector<Sensor> getSensors(const string& token) {
    ...
    if (curl) {
        CURLcode res;
        response_data.clear();
        curl_easy_setopt(curl, CURLOPT_URL, Config::SENSOR_API.c_str());
        struct curl_slist* headers = NULL;
...
vector<Location> getLocations(const string& token) {
    ...
    if (curl) {
        CURLcode res;
        response_data.clear();
        curl_easy_setopt(curl, CURLOPT_URL, Config::LOCATION_API.c_str());
        struct curl_slist* headers = NULL;
        string auth_header = "Authorization: Bearer " + token;
        headers = curl_slist_append(headers, auth_header.c_str());
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
...
double calculateRadiationLevel(double latitude, double longitude, time_t lastUpdate) {
    double baseRadiation = Config::LEVEL_LOW; 
    ...
    if (randomChance < 5) { 
        radiationLevel = Config::LEVEL_CRITICAL + (rand() % 10) / 100.0; 
    }
    else if (randomChance < 15) {
        radiationLevel = Config::LEVEL_HIGH + (rand() % 5) / 100.0;
    }
...
void updateSensorRadiation(std::vector<Sensor>& sensors, const std::vector<Location>& locations, const std::string& token) {
    ...
    for (auto& sensor : sensors) {
        ...
        if (location != locations.end()) {
            ...
            data["alert_triggered"] = Config::LEVEL_HIGH <= radiationLevel ? true : false;

            std::string data_str = data.dump();

            curl_easy_setopt(curl, CURLOPT_URL, Config::RADIATION_API.c_str());
...
int main() {
    string token = getJwtAcces(Config::EMAIL, Config::PASSWORD);
    ...
    while (true) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(Config::SENSOR_UPDATE_INTERVAL_MS)); 
    }
